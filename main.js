...
const listSection = document.getElementById("name-list");
const musicSection = document.getElementById("music-list");
...
function getBabyName() {
  ...
  // Reset lists to clear any previous data
  listSection.innerText = "";
  musicSection.innerText = "";
  ...
  fetch(babyURL, { headers: { "X-Api-Key": API_NINJA_KEY } })
    ...
    .then((data) => {
      ...
      data.forEach((name, index) => {
        ...
        nameElement.id = `artist-${index}`;
        nameElement.innerText = name;
        nameElement.classList.add(gender);
        nameElement.onclick = () => {
          // Only highlight the actively selected name
...
function getMusic(name) {
  ...
  fetch(spotifyURL, {
    ...
    .then((data) => {
      ...
      musicSection.innerHTML = `<h3>Songs Related to "${name}"</h3>`;
